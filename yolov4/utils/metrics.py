...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, fname='precision-recall_curve.png'):
    ...
    tp, fp, p, r, f1, ap, unique_classes, p_curve, r_curve, f1_curve, x, prec_values = ultralytics.utils.metrics.ap_per_class(tp, conf, pred_cls, target_cls, plot=True, save_dir=Path(os.path.dirname(fname)), names={0:'yellow', 1:'red', 2:'blue'})
    ...
    if plot:
        ...
        for i, y in enumerate(f1):
            ax2.plot(px, y, linewidth=1, label=f'{names[i]} {f1[i:]:.3f} at {px[my.argmax()]:.3f}')
        ax2.plot(px, py.mean(1), linewidth=2, color='blue', label=f'all classes {my.max():.3f} at {px[my.argmax()]:.3f}')
        ...
        plt.legend()
        fig2.tight_layout()
        fig2.savefig('yolov4_func_f1_curve.png', dpi=200)
